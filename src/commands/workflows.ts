...
function renderWorkflowList(workflows: WorkflowSummary[]): void {
  ...
  for (const w of workflows) {
    blocks.push(ui.h2(`${w.name} (${ui.code(w.identifier)})`));
    const paramSummary =
      w.parameters.length === 0
        ? ui.dim("(No parameters)")
        : `${w.parameters.length} parameter(s)`;
    blocks.push(
      ui.dl(
        [
          ui.dli("Scope", w.scope),
          ui.dli("Execution", w.execution_type),
          ui.dli("Trigger", w.trigger_type),
          ui.dli(
            "Description",
            w.description?.trim() ? w.description : ui.dim("(None)"),
          ),
          ui.dli("Parameters", paramSummary),
        ],
        { termWidth: 12 },
      ),
    );
