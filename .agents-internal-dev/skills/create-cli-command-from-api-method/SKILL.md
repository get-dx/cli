...
- Add a small typed (or pragmatically typed) function in the same module as the command that calls `request()` from `src/http.ts` with the correct **method**, **path**, **query**, and **body** as in the docs **Facts** / **Example request**.
